          ...
          displayedItems.map((item) => (
            <div key={`${item._id}-${item.size}-${item.color}`} className="bg-slate-200 flex justify-between items-center p-1 shadow">
              <div className="flex items-center">
                <img src={item.image || "https://via.placeholder.com/50"} alt={item.name} className="w-12 h-12 object-cover" loading="lazy" />
                <div className="ml-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button className="px-2 py-1 rounded bg-gray-300 hover:bg-gray-400 transition-colors" onClick={() => handleUpdateQuantity(item, item.quantity + 1)}>
                  +
                </button>

                <button className="hover:text-red-600 text-black px-2 transition-colors" onClick={(event) => handleRemoveItem(item._id, event)}>
