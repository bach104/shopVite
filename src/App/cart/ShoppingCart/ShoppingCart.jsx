    ...
    return (
        <div className="my-4 shoppingCart relative">
            <section className="container-width p-4">
                ...
                {isEditing && (
                    <h3 className="text-center text-xl text-black opacity-80 font-semibold">Chọn sản phẩm để xóa</h3>
                )}
                ...
                <div className="space-y-3 shoppingContainer">
                    {isLoading ? (
                        ...
                        cartData.cartItems.map((item) => (
                            <div key={item._id} className="flex shoppingItems gap-2 h-32 bg__select p-2 rounded-sm mb-3 shadow-sm">
                                <img src={item.image} alt={item.name} className="w-28 border border-black h-full object-cover rounded-s" />
                                <div className="flex-1 shoppingItems__technology">
                                    ...
                                    <p className="text-sm gap-2 font-semibold">
                                        {item.price}đ
                                ...
                                <div className="flex flex-col shoppingItems__click items-end justify-between">
                                    {!isEditing ? (
                                        ...
                                        <input 
                                            type="checkbox" 
                                            className="w-5 h-5 cursor-pointer bg-red-400 checked:bg-red-600"
                                            checked={selectedItems.includes(item._id)}
                                            onChange={() => handleSelectItem(item._id)}
                                        />
                                    )}
                ...
                {isLoggedIn && cartData?.cartItems.length > 0 && (
                    <div className="flex py-4 justify-between items-center">
                        ...
                        <div className="flex items-center space-x-4">
                            {currentPage > 1 && (
                                <p className="font-semibold cursor-pointer hover:text-red-500" onClick={handlePrevPage}>
                                    <FontAwesomeIcon className="pr-2 text-sm" icon={faChevronRight} rotation={180} />
                                    Trang trước
                                </p>
                            )}
                            {cartData?.currentPage < cartData?.totalPages && (
                                <p className="font-semibold cursor-pointer hover:text-red-500" onClick={handleNextPage}>
                                    Trang tiếp theo
                ...
                <CartSummary totalQuantity={totalQuantity} totalPrice={totalPrice} onClearSelection={handleClearSelection} />
            </section>

            {showUpdate && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center">
                    <CartUpdate onClose={() => setShowUpdate(false)} />
                </div>
            )}
        </div>
    );
